...
const SendFeedback = ({ userid }) => {
  ...
  const handleSubmit = () => {
    if (!feedbackType || !feedbackSubtype || feedbackText.length < 50) {
      setErrorMessage(
        "Please fill in all required fields and provide at least 50 characters of feedback."
      );
      return;
    }

    const feedbackData = {
      regno: anonymous ? null : userid,
      canteen: feedbackType === "food" ? feedbackSubtype : null,
      food: feedbackType === "food" ? feedbackSubtype : null,
